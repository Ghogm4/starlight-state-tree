...
public partial class STRoot : STNode
{
    ...
    protected void RegisterStateRecursively(STNode state)
    {
        ...
        {
            GD.PushError($"Duplicate state name '{state.Name}' found at {state.GetPath()}. State names must be unique under root '{Name}'.");
            state.TransitionRequested -= OnStateTransitionRequested;
            return;
        }
        ...
        {
            if (child is STNode childState)
            {
                RegisterStateRecursively(childState);
                continue;
            }
            GD.PushWarning($"Child node '{child.Name}' of STNode '{state.Name}' is not an STNode and will be ignored.");
        }
    }
    protected void OnStateTransitionRequested(string targetStateName)
    {
        CallDeferred(MethodName.TransitionTo, targetStateName);
    }
